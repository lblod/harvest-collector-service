...
/*
  Global TODOs 
    1. Get rid of the collection statuses, they are not useful
    2. Ensure we still need to use getFirstNotStartedFile. Maybe we could now always use the file
      that enters processFile and simplify the code accordingly ?
*/
...
const REMOTE_NOT_STARTED_STATUS = 'http://lblod.data.gift/file-download-statuses/not-started';
...
async function ensureFilesAreReadyForHarvesting(remoteFiles) {
  // TODO - Use map(escapeUri).join(',') insead of (${sparqlEscapeUri(remoteFiles.join(">, <"))})
  const result = await query(`
    ...
    WHERE {
      ...
      FILTER( ?remoteDataObject IN (${sparqlEscapeUri(remoteFiles.join(">, <"))})) .
...
async function processFile(file, collection) {
  ...
  try {
    ...
    if (physicalFile) {
      ...
      if (urls.length) {
        await prepareNewDownloads(urls, file, collection);
      }

      // TODO - Reformulate this comment
      // Getting the first file with not-started status prevent sending duplicated deltas
      const firstNotStartedFile = await getFirstNotStartedFile(collection);
      if (firstNotStartedFile) {
        console.log(`Waiting for the next download of file ${firstNotStartedFile}`)
        await updateHarvestStatus(firstNotStartedFile, REMOTE_READY_STATUS);
...
*/
async function prepareNewDownloads(urls, originatingfile, collection) {
  for (let url of urls) {
      ...
      INSERT DATA {
        GRAPH <http://mu.semte.ch/graphs/public> {
          ...
          ${sparqlEscapeUri(remoteDataObjectUri)} a nfo:RemoteDataObject, nfo:FileDataObject;
            ...
            adms:status ${sparqlEscapeUri(REMOTE_NOT_STARTED_STATUS)};
...
async function collectionHasStatus(collection, status) {
  const result = await query(`
    PREFIX adms: <http://www.w3.org/ns/adms#>
    ...
    WHERE {
      GRAPH ?g {
        ${sparqlEscapeUri(collection)} adms:status ${sparqlEscapeUri(status)}.
      }
    }
  `);
...
async function getFirstNotStartedFile(collection) {
  const result = await query(`
    PREFIX adms: <http://www.w3.org/ns/adms#>
    PREFIX dct: <http://purl.org/dc/terms/>
    ...
    WHERE {
      GRAPH ?g {
        ...
        ?file adms:status ${sparqlEscapeUri(REMOTE_NOT_STARTED_STATUS)} .
      }
    } LIMIT 1
  `);
