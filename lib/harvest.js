...
async function processFile(file, collection) {
  ...
  `);
  const task = taskResult.results.bindings[0]['task'].value;
console.log("task", task);
  ...
  try {
    ...
    if (result.results.bindings.length) {
      const filePath = result.results.bindings[0]['physicalFile'].value.replace('share://', FILE_BASE_DIR);
      ...
      urls = await ensureFullUrls(urls, file);
