...
import { attachClonedAuthenticationConfiguraton, hasAuth } from './credential-helpers';
  ...
  try {
    ...
    if (physicalFile) {
      ...
      if (urls.length) {
        ...
        await prepareNewDownloads(urls, collection, remoteDataObject);
 ...
 *
 * This is also handling if the remoteDataObject needs authentication.
*/
async function prepareNewDownloads(urls, collection, remoteDataObject) {
  for (let url of urls) {
    if (!(await hasBeenCollected(url, collection))) {
      ...
      `;
      // Making sure we attach the authConf only when needed
      if (hasAuth(remoteDataObject)) {
        const newAuthConf = `http://data.lblod.info/id/authentication-configurations/${uuid()}`;

        await update(createNewRemoteDataObjectQuery);
        await attachClonedAuthenticationConfiguraton(remoteDataObjectUri, remoteDataObject, newAuthConf);
      } else {
        await update(createNewRemoteDataObjectQuery);
      }
    }
