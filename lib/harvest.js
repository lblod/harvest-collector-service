...
async function appendCollectedFilesToTaskResultsContainer(task, collection) {
  ...
  if (result.results.bindings.length) {
    for (let r of result.results.bindings){
        const insertRequest = `
          PREFIX dct: <http://purl.org/dc/terms/>
          PREFIX task: <http://redpencil.data.gift/vocabularies/tasks/>
          PREFIX nfo: <http://www.semanticdesktop.org/ontologies/2007/03/22/nfo#>
          ...
          PREFIX nie: <http://www.semanticdesktop.org/ontologies/2007/01/19/nie#>
          INSERT DATA {
            GRAPH ${sparqlEscapeUri(r['g'].value)} {
              ...
              ${sparqlEscapeUri(containerUri)} task:hasFile ${sparqlEscapeUri(r['remoteDataObject'].value)}.
              ${sparqlEscapeUri(r['remoteDataObject'].value)} a nfo:FileDataObject.
              ${sparqlEscapeUri(r['remoteDataObject'].value)} nfo:fileName ${sparqlEscapeString(r['fileName'].value)}.
              ${sparqlEscapeUri(r['remoteDataObject'].value)} nfo:fileSize ${sparqlEscapeString(r['size'].value)}.
            }
          }
