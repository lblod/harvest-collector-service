...
const TASK_STATUS_SUCCESS = 'http://lblod.data.gift/id/lblodJob/concept/JobStatus/success';
const TASK_STATUS_FAILED = 'http://lblod.data.gift/id/lblodJob/concept/JobStatus/failed';
...
async function getTask(collection) {
  const taskResult = await query(`
    ...
    WHERE {
      GRAPH ?g {
        ?task lblodJob:inputContainer ${sparqlEscapeUri(collection)}.
      }
    }
  `);
...
async function appendCollectedFilesToTaskResultsContainer(task, collection){
  const queryStr = `
    ...
    INSERT {
      GRAPH ?g {
        ${sparqlEscapeUri(task)} lblodJob:resultsContainer ?remoteDataObject.
      }
    }
    WHERE {
      GRAPH ?g {
